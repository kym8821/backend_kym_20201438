...
route.get('', async (req, res) => {
  ...
  if (result === undefined) return res.status(500).send(getCommonResponse(res.statusCode, 'sql error'));
...
route.get('/:projectId', async (req, res) => {
  ...
  if (project === undefined) return res.status(500).send(getCommonResponse(res.statusCode, 'sql error'));
