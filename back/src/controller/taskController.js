...
route.post('/:projectId/tasks', async (req, res) => {
  ...
  if (isNaN(pjId) || !isTaskSaveDto(saveDto) || (await projectService.findById(pjId)).length === 0)
...
route.put('/:projectId/tasks/:taskId', async (req, res) => {
  ...
  console.log(updateDto, pjId, taskId);
  if (isNaN(pjId) || isNaN(taskId) || !isTaskUpdateDto(updateDto)) return res.status(400).send(getCommonResponse(res.statusCode, 'invalid request'));
...
route.delete('/:projectId/tasks/:taskId', async (req, res) => {
  ...
  if (result > 0) return res.status(200).send(getCommonResponse(200, 'success'));
