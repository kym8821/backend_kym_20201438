...
async function findByTaskAndPj(conn, pjId, id) {
  const query = "SELECT * FROM task WHERE id=? AND pjId=?";
  const param = [id, pjId];
  const result = await conn.query(query, param);
  return result[0];
}
...
const taskRepository = { save, findByPjId, findById, update, deleteById, findByTaskAndPj };
