...
async function save(saveDto) {
  const saveAndGet = async (conn, dto) => {
    ...
    await projectRepository.updateProject(conn, project.id, new ProjectUpdateDto(undefined, undefined, JSON.stringify(project.tasks.concat([insertId]))));
...
async function updateTask(pjId, taskId, updateDto) {
  const updateAndGet = async (conn, dto) => {
    await taskRepository.update(conn, pjId, taskId, dto);
    const updatedTask = await taskRepository.findById(conn, taskId);
    return updatedTask[0];
  };
  const result = await transactionManager(connection, updateAndGet, [updateDto]);
  return result;
}
// not transaction : delete task by task id
async function deleteById(pjId, taskId) {
  const deleteAndUpdate = async (conn, pid, tid) => {
    ...
    if (deleteResult.affectedRows > 0 && projects.length > 0) {
      ...
      await projectRepository.updateProject(conn, pid, new ProjectUpdateDto(project.title, project.description, JSON.stringify(project.tasks)));
    }
